...
public class RuleRepository : IRuleRepository
{
    ...
    public async Task<IEnumerable<RuleModel>> GetAllRules(PaginationFilter filter, CancellationToken cts)
    {
    ...
    {
        const string sql = @"INSERT INTO rule (rule, owner_user_id, issue_type_id, 
                                               analysis_area_id, is_regex, is_negative)
                                               VALUES 
                                               (@Rule, @OwnerUserId, @IssueTypeId,
                                               @AnalysisAreaId, @IsRegex, @IsNegative)
                                               RETURNING *";
    ...
    {
        const string sql = @"UPDATE rule SET
                             rule = @Rule,
                             owner_user_id = @OwnerUserId,
                             issue_type_id = @IssueTypeId,
                             analysis_area_id = @AnalysisAreaId,
                             is_regex = @IsRegex,
