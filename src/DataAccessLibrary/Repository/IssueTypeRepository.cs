...
public class IssueTypeRepository : IIssueTypeRepository
{
    ...
    public async Task<IEnumerable<IssueTypeModel>> GetAllIssueTypes(PaginationFilter filter, CancellationToken cts)
    {
    ...
    {
        const string sql = @"INSERT INTO issue_type (issue_type_name) VALUES (@IssueTypeName) RETURNING *";
    ...
    public async Task<IssueTypeModel> ChangeIssueTypeName(IssueTypeModel issueType, CancellationToken cts)
    {
        const string sql = @"UPDATE issue_type SET issue_type_name = @NewIssueTypeName WHERE id = @Id RETURNING *";
    ...
    {
        const string sql = @"SELECT EXISTS (SELECT true FROM issue_type WHERE issue_type_name = @IssueTypeName)";
